...
class FirestoreRest {
  ...
  /// Follows pagination until the collection is exhausted.
...
/// Dart JSON → the "typed value" format of the Firestore REST API.
Map<String, dynamic> encodeValue(Object? v) => switch (v) {
  ...
  _ => throw ArgumentError('unhandled type: ${v.runtimeType}'),
};
...
Object? decodeValue(Map<String, dynamic> v) {
  ...
  throw ArgumentError('unhandled value: ${v.keys}');
}
...
Future<String> gcloudToken() async {
  ...
  if (r.exitCode != 0) {
    stderr.writeln('gcloud auth print-access-token failed: ${r.stderr}');
    exit(1);
  }
